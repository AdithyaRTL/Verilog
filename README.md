...
1. iverilog -o fa_sim.vvp full_adder.v full_adder_tb.v

2. vvp fa_sim.vvp

3. gtkwave full_adder_tb.vcd
...
a. $dumpfile("your_name.vcd"); and $dumpfile_vars / $dumpvars(0, tb_name); are inside the initial block of your testbench.

b. The .vcd filename in Command 3 matches whatever name you passed to $dumpfile() in your testbench.

c. You are running the commands inside cmd (Command Prompt), not PowerShell.
...
Here is the exact breakdown:
    ...
    i. The 4 Changes to Make
       ...
       a. read_verilog <YOUR_NEW_FILE.v>          # Change 1: Your input Verilog filename
       
       b. hierarchy -top <YOUR_MODULE_NAME>       # Change 2: The top module name inside your code
       
       c. proc; opt; techmap; opt
       
       d. write_verilog <NEW_SYNTH_FILE.v>        # Change 3: Output synthesized file name
       
       e. show -format dot -prefix <NEW_SCHEMATIC> # Change 4 & Tweak: Output schematic prefix + 'dot' format
       
    ii. Example: Testing a Full Adder
    
       If you create a file named full_adder.v with module full_adder(...):
       
       a. read_verilog full_adder.v
       
       b. hierarchy -top full_adder
       
       c. proc; opt; techmap; opt
       
       d. write_verilog full_adder_synth.v
       
       e. show -format dot -prefix full_adder_schematic
...
4. Open & View:
    ...
    Click the generated .dot file in VS Code's sidebar and hit Ctrl + Shift + V to preview the diagram.
